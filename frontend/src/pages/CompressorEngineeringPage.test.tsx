...
describe("CompressorEngineeringPage", () => {
  ...
  it("renders the active project and ordered engineering workflow", () => {
    ...
    expect(steps[6]).toHaveTextContent("Step 7 · Detailed Engineering");
    expect(steps[6]).toHaveTextContent("Distribution Network");
    expect(within(workflow).getAllByText("Ready")).toHaveLength(7);
  });
  ...
  it("provides project-scoped links for every engineering workspace", () => {
    ...
    expect(
      screen.getByRole("link", { name: /Distribution Network/ }),
    ).toHaveAttribute("href", "/projects/42/compressor/distribution");
  });
